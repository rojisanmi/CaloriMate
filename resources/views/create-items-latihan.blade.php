...
<div class="max-w-xl mx-auto cm-fadein">
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      <h1 class="font-raleway text-xl font-bold text-[#2E471F]">Tambah Item Latihan</h1>
  ...
  <div class="bg-white rounded-2xl shadow-sm p-6">
    <form method="POST" action="{{ route('trainer.programs.items.store', $program) }}" class="space-y-5">
      @csrf

      <div>
        <label for="exercise_name" class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Latihan</label>
        <input id="exercise_name" name="exercise_name" type="text"
               value="{{ old('exercise_name') }}" required
      ...
      <div>
        <label for="duration_minutes" class="block text-sm font-semibold text-gray-700 mb-1.5">
          Durasi (menit)
        </label>
        <input id="duration_minutes" name="duration_minutes" type="number" min="0"
               value="{{ old('duration_minutes') }}" placeholder="mis. 10"
      ...
      <div>
        <label for="intensity_level" class="block text-sm font-semibold text-gray-700 mb-1.5">
          Tingkat Intensitas
        </label>
