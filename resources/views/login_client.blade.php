  ...
  <div class="auth-card rounded-3xl overflow-hidden mx-auto w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 shadow-xl bg-white">
    ...
    <div class="relative bg-white p-8 md:p-10 lg:p-12 flex items-center justify-center">
      ...
      <div class="w-full max-w-sm text-center pt-12 md:pt-28 lg:pt-20">
        <h2 class="font-raleway text-3xl md:text-4xl font-bold text-[#2E471F] mb-6">Login Client</h2>
        ...
        <form method="POST" action="{{ route('login.do') }}" class="space-y-5">
          @csrf
          <label class="block">
            <span class="block mb-1 text-sm font-semibold text-[#2E4F2A]">Username</span>
            <input type="text" name="username" placeholder="Username" value="{{ old('username') }}" required
              class="w-full px-6 py-4 border-2 {{ $errors->has('login') ? 'border-red-400 bg-red-50' : 'border-gray-200' }} rounded-xl text-gray-700 placeholder-gray-400 text-base focus:border-[#2E4F2A] focus:outline-none transition">
          </label>
          <label class="block">
            <span class="block mb-1 text-sm font-semibold text-[#2E4F2A]">Password</span>
